function  [ P ] = penaltyImplicitAmerican( S, tau, E, r, sigma, varargin )
% penaltyImplicitAmerican: Computes the fair value of the American put option
% using the implicit Euler method. Uses the penalty method to solve the linear
...
%     2.     x - g >= 0
%     3. (x-g) * (A * x - b) = 0
% In general, with the penalty method, we are solving a nonlinear equation at
% every iteration.
% INPUT PARAMETERS
...
% matrix trick: column vector * row vector (outer product)
...
%    % G(i, 2:numPartitionsX)
%    % PRICE()
%    PRICE(i, 2:numPartitionsX) = ...
%        transpose(psor(TRI, rhs, PRICE_prev, ...
%                       transpose(G(i,2:numPartitionsX))));

%    zeta_vec = arrayfun(zeta_func, G(i, 2:numPartitionsX), PRICE(i, 2:numPartitionsX));
